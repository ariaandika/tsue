pub fn source_iter() -> impl Iterator<Item = SourceLine> {
    SOURCE_STRING.lines().skip(1).filter_map(|e| SourceLine::new(e.as_bytes()))
}
...
impl SourceLine {
    /// Returns `None` on EOS entry.
    fn new(line: &'static [u8]) -> Option<Self> {
        match str::from_utf8(line[SYM_RANGE].trim_ascii_start())
            .unwrap()
            .parse()
        {
            Ok(byte) => Some(Self { sym: byte, line }),
            Err(err) => {
                if line[SYM_RANGE].eq(b"256") {
                    // EOS
                    None
                } else {
    ...
    pub fn bits(&self) -> Bits {
        Bits {
            ...
            iter: Box::new(self.line[BITS_RANGE].iter().copied()),
            entry: self.clone(),
        }
    }

    pub fn bits_len(&self) -> usize {
        str::from_utf8(self.line[BITS_LEN_RANGE].trim_ascii_start())
            .unwrap()
            .parse()
            .unwrap()
    }
...
impl Bits {
    ...
    pub fn into_shifted(self, shift: &'static [u8]) -> Bits {
        Bits {
            remaining: shift.len() + self.remaining,
            iter: Box::new(shift.iter().copied().chain(self.iter)),
            entry: self.entry,
        }
    }
