...
status_code_v3! {
    ...
    /// The client's identity is known to the server, but client does not have access rights to the
    /// content, but t
    ...
    /// The user has sent too many requests in a given amount of time ([rate limiting][1]).
    ///
